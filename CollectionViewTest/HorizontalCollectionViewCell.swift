...
extension HorizontalCollectionViewCell: UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        NSLog("\(String(describing: Self.self))::\(#function)@line:\(#line)"
            + "\nSelf.defaultCellDimensions: \(Self.defaultCellDimensions)"
            + "\ncollectionView.contentSize: \(collectionView.contentSize)"
//            + "\ncollectionViewLayout: \(collectionViewLayout)"
        )
        let tempViewForCalculateCellDimension = UILabel()
        tempViewForCalculateCellDimension.text = contents[indexPath.row].labelText
